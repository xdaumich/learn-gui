...
SERVICE_NAME="telemetry-console"
...
cmd_install() {
    ...
    ok "Service installed. Run: camera_service.sh --start"
}
...
cmd_start() {
    if [[ ! -f "$DEST_UNIT" ]]; then
        err "Service not installed. Run: camera_service.sh --install"
        exit 1
    fi
    need_sudo
...
usage() {
    cat <<EOF
Usage: camera_service.sh <command>
